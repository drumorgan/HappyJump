...
- **Counting:** generic `countItemUseInLog(apiKey, itemId, drugName, sinceTs, untilTs)` helper in the gateway, paginated against the Torn user log endpoint, reusing `entryMatchesDrugUse` so buys/trades/ODs aren't counted as uses. Attack events use `countAttacksInWindow(apiKey, myId, sinceTs, untilTs)` instead, which paginates the `attacks` selection and returns `{ count, respect_total, best_single }`. **Chain-bonus stripping:** the attack scanner divides each hit's `respect_gain` by its `modifiers.chain_bonus` multiplier so milestone chain bonuses (100/250/500/1000/2500/5000/10000/25000/50000/100000) don't dominate the leaderboard — a hit that earned 3 base respect amplified to 103 by a chain bonus counts as 3, not 103. Normal hits have `chain_bonus = 1` so they pass through unchanged. The `fe-test-count-attacks` diagnostic surfaces both raw and base totals so the operator can validate the math against actual Torn data. Both scanners are dispatched from a single `runEventScanner(apiKey, event, tornId, fromSec, untilSec)` helper that every refresh path (`create-faction-event`, `join-faction-event`, `refresh-faction-event`, `refresh-stale-participants`, `admin-rescrape-participant`, `force-refresh-all-participants`, `scrape-next-pending`) routes through.
- **Event types:** `faction_events.event_type` is `drug_use` (default) or `attacks` (migration `017_faction_event_attacks.sql`). `drug_use` events show one column (count of uses); `attacks` events show three sortable columns (# attacks · respect gained, default sort · avg respect per attack), plus the single best-respect hit as a sub-line. Drug fields (`drug_item_id` / `drug_name`) are nullable and meaningless on attack events; the edit-drug pencil is hidden and `update-faction-event` rejects drug edits for them. Attack metrics are stored on `faction_event_participants` as `last_respect_total` and `last_best_single_respect` (both numeric, null on drug events).
- **Attacks API key scope:** the Custom Key minting link includes `attacks` in addition to `basic,profile,log,calendar`. Keys minted before that was added still work for drug events but error out (Torn code 16) on attack scans — the scanner surfaces `scopeError`, all callers preserve the participant's prior `last_count` rather than zeroing it, and the participant needs to re-mint with a wider Custom Key for counts to resume.
- **Event window:** drug events anchor at **10:00 TCT** (Torn City Time = UTC) on a chosen start date; attack events use a **creator-set exact start time** (HH:MM TCT) on a chosen date. Both run for a per-event **duration in hours** (default 48h, cap 720h / 30 days). The create / edit forms expose a date picker + a duration input; attack events additionally expose a time picker (hidden for drug events). `starts_at` is `<date>T<HH:MM>:00Z` and `ends_at` is `starts_at + duration_hours`. Backend stores both as `timestamptz`; the window is otherwise unconstrained.
- **Personal start time (drug events only):** each drug-event participant picks one of **25 fixed 15-minute TCT slots** (10:00, 10:15, … 16:00) **on the event's start date** — regardless of how long the event runs. Each participant gets the **full event duration** anchored to their own personal start. Pick 12:00 TCT on a 48h event starting Saturday and your count window is `[Sat 12:00 TCT, Mon 12:00 TCT]` (i.e. personal_start + 48h). The count window is `[max(personal_start, event.start), min(now, personal_start + duration)]` where `duration = event.ends_at − event.starts_at`. Crucially, the upper bound is NOT capped at `event.ends_at` — a participant whose personal_start is 6h after event_start still gets the full 48h, ending 6h past the nominal event end. `event.ends_at` exists only to define the duration. Default selection is the slot at-or-before now in TCT (clamped into 10:00-16:00). Best-effort autofill via the `fetch-torn-event-start` action probes `selections=calendar` and snaps the picker to the closest TCT slot — picker is source of truth. Personal start time is **not** part of the create-event form — the creator's row defaults to event_start, then they (and every joiner) pick / change their slot from the me-card "Change my start time" UI. **Attack events ignore personal_start_at entirely** — every participant uses the global `[event.starts_at, event.ends_at]` window, the join slot picker is hidden, the calendar pull is hidden, the "Change my start time" button is hidden, and the gateway hard-pins `personal_start_at = event.starts_at` server-side regardless of what the client supplies.
- **FE session (Faction Event auth):** independent from the Happy Jump session — its own table (`faction_event_player_secrets`), its own localStorage entry (`faction_event_session`), its own gateway actions (`fe-set-api-key`, `fe-auto-login`, `fe-revoke-session`). FE keys need `basic,profile,log,attacks` (calendar is best-effort) and grant only the right to count drug uses or attacks + edit your own events. Cascade-delete on permanent Torn errors (codes 2/16) drops the secret row AND every participant row anchored on that torn_id; transient errors (5/8/9) leave the row alone. Signing out of FE does NOT sign you out of Happy Jump and vice-versa.
- **Creator edits:** the user whose `torn_id` matches `event.creator_torn_id` sees pencil buttons next to title / drug / window in the event header. Edits route through `update-faction-event` (FE-creator-authorized). Drug or window changes invalidate every participant row (`last_checked_at = NULL`) so the next sweep recounts; window changes also **re-anchor** personal-start times to the new event date — each participant's TIME-OF-DAY is preserved, so a date change moves their start by exactly the amount the event moved. If a participant's old time-of-day predates the new event_start, their personal_start snaps to event_start (no upper-bound snap — under the `personal_start + duration` count model, a personal_start past the nominal event_end is fine; the participant just ends up with their full duration window shifted later).
...
- **Auto-add faction members:** every successful `fe-set-api-key` / `fe-auto-login` caches the player's `torn_name` + `torn_faction_id` + `torn_faction_name` on `faction_event_player_secrets` (migration `015_faction_event_secret_faction.sql`). When `create-faction-event` runs, after seeding the creator's participant row it bulk-inserts a participant row for every secret whose `torn_faction_id` is in the request's `auto_seed_faction_ids` array (creator excluded) at `personal_start_at = event_start`, `last_count = 0`, `last_checked_at = NULL` — the next `refresh-stale-participants` sweep is what actually runs each member's count, using their stored key. The create form populates `auto_seed_faction_ids` via a checkbox list ("Auto-seed factions") sourced from `list-fe-factions`, which returns distinct factions among FE secrets with member counts and marks the caller's own faction `mine: true` so the UI pre-checks it. Skipped silently for: members who never logged in (no secret row), members in a faction the creator didn't tick, members with NULL cached `torn_name` (rows from before migration 015 — they get auto-added once they re-validate), and creators who themselves have no faction (legacy default behaviour). Multi-faction events are supported — the creator just ticks every faction they want to invite. Auto-seed only fires for new events; existing events are not retro-populated. The creator (or HJ admin) prunes stragglers with the per-row `×` button on the leaderboard, which calls `remove-faction-event-participant`. When the `auto_seed_faction_ids` field is omitted (legacy clients), the gateway falls back to `[creator.torn_faction_id]` — single-faction parity with the old behaviour.
- **Gateway actions:** `fe-set-api-key`, `fe-auto-login`, `fe-revoke-session`, `create-faction-event` (FE auth required; auto-seeds creator + all faction-mate secrets), `get-faction-event`, `list-faction-events`, `update-faction-event` (FE creator only), `delete-faction-event` (FE creator OR HJ admin), `remove-faction-event-participant` (FE creator OR HJ admin), `join-faction-event`, `refresh-faction-event`, `refresh-stale-participants`, `fetch-torn-event-start`. Events identified by UUID v4, ~122 bits of entropy in the share link.
...
| `list-fe-factions`       | None (FE auth optional) | Distinct factions among FE-signed-in users with member counts; flags caller's own faction |
...
- **Price snapshots:** Always snapshot pricing at time of transaction creation — config changes mid-week must not affect open transactions. The gateway's `create-transaction` action fetches **live Torn market values** (Xanax 206, EDVD 366, Ecstasy 197) via the user's own API key/session when auth is supplied, so the locked-in snapshot matches what the client saw on screen (which also uses live Torn prices, not stored config). If the Torn lookup fails or no auth is present, it falls back to `config.*_price` so a Torn API blip never blocks a purchase. `createTransaction()` in `src/api.js` passes `currentAuth()` (session or raw key) through for this reason.
