...
- **Auto-add faction members:** every successful `fe-set-api-key` / `fe-auto-login` caches the player's `torn_name` + `torn_faction_id` + `torn_faction_name` on `faction_event_player_secrets` (migration `015_faction_event_secret_faction.sql`). When `create-faction-event` runs, after seeding the creator's participant row it bulk-inserts a participant row for every other secret with the **same** `torn_faction_id` (creator excluded) at `personal_start_at = event_start`, `last_count = 0`, `last_checked_at = NULL` — the next `refresh-stale-participants` sweep is what actually runs each member's count, using their stored key. Skipped silently for: members who never logged in (no secret row), members in a different faction, members with NULL cached `torn_name` (rows from before migration 015 — they get auto-added once they re-validate), and creators who themselves have no faction. Auto-seed only fires for new events; existing events are not retro-populated. The creator (or HJ admin) prunes stragglers with the per-row `×` button on the leaderboard, which calls `remove-faction-event-participant`. Multi-faction events are not handled yet — a future iteration may take a list of faction_ids on the create form.
