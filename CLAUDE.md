...
- **Event window:** events anchor at **10:00 TCT** (Torn City Time = UTC) on a chosen start date and run for a per-event **duration in hours** (default 6h, cap 720h / 30 days). The create / edit forms expose a date picker + a duration input; `starts_at` is `<date>T10:00:00Z` and `ends_at` is `starts_at + duration_hours`. Backend stores both as `timestamptz`; the window is otherwise unconstrained.
